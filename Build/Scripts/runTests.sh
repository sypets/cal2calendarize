...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/cal2calendarize/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/cal2calendarize/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/cal2calendarize/Tests/Unit"
            ;;
    esac
fi
